<template>
  <div class="requisition-order-list">
      ...
      <el-form :model="query" inline @submit.prevent="handleSearch">
        ...
        <el-form-item label="状态">
          <el-select v-model="query.status" placeholder="请选择" clearable style="width: 140px">
            <el-option
              v-for="item in getDictList('requisitionOrderStatus')"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            />
    ...
    <el-card shadow="never">
      ...
      <el-table v-loading="loading" :data="list" border stripe>
        ...
        <el-table-column prop="orderNo" label="领料单号" min-width="140" />
        ...
        <el-table-column prop="status" label="状态" width="100" align="center">
          <template #default="{ row }">
            <el-tag :type="getDictType('requisitionOrderStatus', row.status || '') as any">
              {{ getDictLabel('requisitionOrderStatus', row.status || '') }}
            </el-tag>
          </template>
        </el-table-column>
    ...
    >
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="总数量" prop="totalQty">
          <el-input-number v-model="form.totalQty" :min="0.0001" :precision="4" style="width: 100%" />
        </el-form-item>
...
import { requisitionOrderApi } from '@/api/material-mgmt/requisitionOrder'
import { materialApi } from '@/api/basic/material'

const loading = ref(false)
const list = ref<RequisitionOrderVO[]>([])
const total = ref(0)
const query = reactive<RequisitionOrderQuery>({
  orderNo: '',
  materialCode: '',
  status: '',
  pageNum: 1,
...
const form = reactive<RequisitionOrderDTO>({
...
async function loadList() {
  loading.value = true
  try {
    const res = await requisitionOrderApi.page(query)
    list.value = res?.list ?? []
...
function handleEdit(row: RequisitionOrderVO) {
...
async function handleSubmit() {
  ...
  try {
    if (editId.value) {
      await requisitionOrderApi.update(editId.value, form)
      ElMessage.success('修改成功')
    } else {
      await requisitionOrderApi.create(form)
...
function handleDelete(row: RequisitionOrderVO) {
  ElMessageBox.confirm('确定要删除该领料单吗？', '提示', {
    type: 'warning',
  }).then(async () => {
    await requisitionOrderApi.delete(row.id)
